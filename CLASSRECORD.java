...
public class JavaApplication5 {
    ...
    public static void main(String[] args) { 
        ...
        System.out.printf("%10s %13s %10s %7s %7s %7s %10s %n", "ID Number","Last Name", "Name", "MI", "Mid", "Fin", "Remarks");
        
        for(int i = 0; i < nS; i++){
            System.out.printf("%10s %13s %10s %7s %7s %7s %10s %n", idNumber[i], lastName[i], firstName[i], middleInitial[i], FmidtermGrade[i], grade[i], status[i]);
        }
        System.out.println();
        String ans;
        boolean valid;
